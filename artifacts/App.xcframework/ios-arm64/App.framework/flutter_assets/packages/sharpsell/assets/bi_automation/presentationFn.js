...
function evaluateCustomExpression(form,user,output) {
//CODE START HERE
{customExpression}
//CODE END HERE
}
function evaluateExpression(form,user,output) {
    let expressionResult = evaluateCustomExpression(form, user, output);
    let result = JSON.stringify(expressionResult);
        if (typeof expressionResult == "string" && result.startsWith('"')) {
